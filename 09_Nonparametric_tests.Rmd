...
The `wilcox.test()` function will conduct our Mann-Whitney-U tests.  We use formula notation and separately defined the data frame object using the `data = ` keyword argument.  Note that we could also have used the `df$CRP ~ df$Group` syntax omitting the second argument.
...
Below we create data for a placebo groups and for two treatment groups, let's say a low-dose and a high-dose group.
...
Now for the Spearman correlation test using the `method = ` argument for the `cor.test()` function to `"Spearman"`.
